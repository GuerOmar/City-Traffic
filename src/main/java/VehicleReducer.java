...
public class VehicleReducer extends Reducer<LongWritable, Sensor,Text,Text> {
    ...
    protected void cleanup(Context context) throws IOException, InterruptedException {
        ...
        context.write(new Text("#Vehicule"), new Text(text));
    ...
    public void reduce(LongWritable key, Iterable<Sensor> values,Context context) throws IOException, InterruptedException {
        for (Sensor sensor: values) {
            counter ++;
            if(sensor.categorie.equals(""))
                continue;
            String ch = sensor.categorie;
